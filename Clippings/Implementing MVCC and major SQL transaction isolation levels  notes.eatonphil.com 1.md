...
All code is [available](https://github.com/eatonphil/gomvcc) on GitHub.
...
The topic is so complex that we've got decades of research papers [critiquing](https://15721.courses.cs.cmu.edu/spring2019/papers/02-transactions/p1-berenson.pdf) SQL isolation levels,[categorization](https://pmg.csail.mit.edu/papers/icde00.pdf) of common isolation anomalies, walkthroughs of anomalies by Martin Kleppmann in [Designing Data-Intensive Applications](https://dataintensive.net/), Martin Kleppman's [Hermitage](https://github.com/ept/hermitage) project documenting common anomalies across isolation levels in major databases, and the [ACIDRain paper](http://www.bailis.org/papers/acidrain-sigmod2017.pdf) showing isolation-related bugs in major open-source ecommerce projects.
...
Transaction isolation levels are basically vibes. The only truth for real projects is Martin Kleppmann's Hermitage project that catalogs behavior across databases. And a truth some people align with is [Generalized Isolation Level Definitions](https://pmg.csail.mit.edu/papers/icde00.pdf).
...
Historically, databases implemented isolation with locking algorithms such as [Two-Phase Locking](https://faculty.cc.gatech.edu/~jarulraj/courses/8803-s22/slides/13-two-phase-locking-annotated.pdf) (not the same thing as [Two-Phase Commit](https://www.cs.princeton.edu/courses/archive/fall16/cos418/docs/L6-2pc.pdf)). Multi-version concurrency control (MVCC) is an approach that lets us completely avoid locks.
...
Here is what the [1999 ANSI SQL standard](https://web.cecs.pdx.edu/~len/sql1999.pdf) (page 84) has to say.

![/sql99isolation.png](https://notes.eatonphil.com/sql99isolation.png)

But as I mentioned in the beginning of the post, we're going to be a bit informal. And we'll mostly refer to [Jepsen](https://jepsen.io/consistency) summaries of each isolation levels.

#### Read Uncommitted

According to [Jepsen](https://jepsen.io/consistency/models/read-uncommitted), the loosest isolation level, Read Uncommitted, has almost no restrictions. We can merely read the most recent (non-deleted) version of a value, regardless of if the transaction that set it has committed or aborted or not.
...
Let's write a test that demonstrates this. We create two transactions,`c1` and `c2`, and set a key in `c1`. The value set for the key in `c1` should be immediately visible if `c2` asks for that key. In main\_test.go:
...
Thank you to @glaebhoerl for [pointing out](https://twitter.com/glaebhoerl/status/1792912649304388043) that in an earlier version of this post, Read Uncommitted incorrectly made deleted values visible.
...
But for the most part, people don't use this isolation level. SQLite, Yugabyte, Cockroach, and Postgres [don't even](https://github.com/ept/hermitage?tab=readme-ov-file#summary-of-test-results) implement it. It is also not the default for any major database that does implement it.
...
This begins to look useful! We will never read a value that isn't part of a committed transaction (or isn't part of our own transaction). Indeed this is the [default](https://github.com/ept/hermitage) isolation level for many databases including Postgres, Yugabyte, Oracle, and SQL Server.
...
Again as Jepsen says, Repeatable Read is the same as Read Committed but with the following anomaly not allowed (quoting from the [ANSI SQL 1999 standard](https://web.cecs.pdx.edu/~len/sql1999.pdf)):
...
There are two major projects I'm aware of that help you test transaction implementations: [Elle](https://github.com/jepsen-io/elle) and [Hermitage](https://github.com/ept/hermitage). These are probably where I'd go looking if I were implementing this for real.
...
This is a real-world issue in Postgres that was [called out](https://ottertune.com/blog/the-part-of-postgresql-we-hate-the-most) by Andy Pavlo and the Ottertune folks. It turns out that Postgres alone among major databases stores the entire value for every version. In contrast, other major databases like MySQL store a diff.
